...
impl AsRef<str> for License {
    fn as_ref(&self) -> &'static str { CORE_GLOBAL_BASE_LICENSE_FEATURE }
}
...
#[derive(Debug)]
#[toolkit_macros::api_dto(response)]
pub struct KitListDto { pub items: Vec<KitDto> }
...
#[derive(Debug)]
#[toolkit_macros::api_dto(response)]
pub struct KitInstallationListDto { pub items: Vec<KitInstallationDto> }
...
impl From<KitDescriptor> for KitDto {
    fn from(value: KitDescriptor) -> Self {
        Self {
            ...
            publisher: value.publisher, visibility: value.visibility, source: value.source,
            repository_url: value.repository_url, default_version: value.default_version,
...
impl From<KitInstallation> for KitInstallationDto {
    fn from(value: KitInstallation) -> Self {
        Self {
            kit_slug: value.kit_slug, version: value.version, source: value.source,
            repository_url: value.repository_url, install_mode: value.install_mode,
            status: value.status, requested_by: value.requested_by,
...
async fn catalogue(
    ...
    Ok(Json(KitListDto { items: service.catalogue().into_iter().map(Into::into).collect() }))
}
...
async fn list_installations(
    ...
    let items = service.list_installations(&ctx, project_id).await.map_err(internal)?;
    Ok(Json(KitInstallationListDto { items: items.into_iter().map(Into::into).collect() }))
}
...
async fn request_installation(
    ...
    let value = service
        .request_installation(&ctx, project_id, &body.kit_slug, &body.version, &body.install_mode)
        .await
        .map_err(|error| KitRegistryError::invalid_argument().with_constraint(error.to_string()).create())?;
    Ok(Json(value.into()))
}
...
async fn remove_installation(
    ...
    service.remove_installation(&ctx, project_id, &kit_slug).await.map_err(internal)?;
...
pub fn register_routes(
    ...
    router = OperationBuilder::get("/studio-kits/v1/catalog")
        ...
        .error_401(openapi).error_500(openapi).register(router, openapi);
    ...
    router = OperationBuilder::get("/studio-kits/v1/projects/{project_id}/installations")
        ...
        .json_response_with_schema::<KitInstallationListDto>(openapi, StatusCode::OK, "Project kit installations")
        .error_401(openapi).error_403(openapi).error_500(openapi).register(router, openapi);
    ...
    OperationBuilder::delete("/studio-kits/v1/projects/{project_id}/installations/{kit_slug}")
        ...
        .error_400(openapi).error_401(openapi).error_403(openapi).error_500(openapi)
